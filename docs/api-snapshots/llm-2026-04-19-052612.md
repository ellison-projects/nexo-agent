...
| `recurring-tasks` | Recurring life-admin items (trash day, insurance, dentist) with a cadence and auto-advancing `next_due_at`. Read-only on the agent API. |
...
Read-only on the agent API. Recurring life-admin items (trash day, insurance renewal, dentist, HVAC filter) with a flexible cadence (every N `day`/`week`/`month`/`year`) and an auto-advancing `next_due_at`. Create, edit, complete, and delete still happen in the web UI — the briefing and this list endpoint are the reads an agent needs to answer "what's due?" and "what did I just finish?".
