...
### "Add trash pickup every Tuesday" / "Set up a recurring reminder"
...
Recurring life-admin items (trash day, insurance renewal, dentist, HVAC filter) with a flexible cadence (every N `day`/`week`/`month`/`year`) and an auto-advancing `next_due_at`. Full CRUD is available, plus a dedicated `/complete` action that marks an item done and advances its next occurrence in one call.

Each row includes `title`, `notes`, `cadence_unit`, `cadence_interval`, `anchor_date`, `next_due_at`, `lead_time_days` (how many days before `next_due_at` reminders start), `escalate_when_overdue` (true = stays surfaced red after the due date; false = a nightly job quietly rolls `next_due_at` forward past today), `category` (free-text), and `last_completed_at`.

| Method | Path | Purpose |
|---|---|---|
| GET | `/api/agent/recurring-tasks?due_before=&due_after=&category=` | List, sorted by `next_due_at` ascending. All three filters are optional; `due_before` / `due_after` expect `YYYY-MM-DD`; `category` is a case-insensitive exact match. |
| POST | `/api/agent/recurring-tasks` | Create. Required: `title`, `cadence_unit`, `cadence_interval`, `anchor_date`. Optional: `notes`, `lead_time_days` (default 3), `escalate_when_overdue` (default false), `category`. `next_due_at` is seeded to `anchor_date`. |
| GET/PATCH/DELETE | `/api/agent/recurring-tasks/{id}` | Detail / partial update / delete. |
| POST | `/api/agent/recurring-tasks/{id}/complete` | Mark done: sets `last_completed_at` to today and advances `next_due_at` by one cadence interval from the later of the current `next_due_at` and today. Use this instead of PATCHing `next_due_at` manually — it's the only path that guarantees the same advancement logic the web UI and nightly job use. |
