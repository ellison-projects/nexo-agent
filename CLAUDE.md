...
- **`telegram-reminder` skill** (`.claude/skills/telegram-reminder/`) — schedules a one-shot Telegram message via the local `at` daemon. **Narrow override** — triggers ONLY when I explicitly name Telegram as the delivery channel ("remind me with telegram in 10 min to X", "telegram me at 3pm about Y"). Every other reminder phrasing routes to `nexo-prm` (PRM's AI reminders are the default reminder system, and they auto-deliver to Telegram via the `nexo-reminders` app — see below). Runs inline (not forked) since it's a single shell command. Uses `at`'s heredoc form to bake `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID` into the spool script at scheduling time, then `curl`s the Telegram Bot API when the job fires. Also handles `atq` / `atrm` for listing and cancelling. Short-term only (minutes to hours); multi-day reminders defer to `nexo-prm`. One-time host setup: `sudo dnf install -y at && sudo systemctl enable --now atd`.
- **`nexo-reminders` pm2 app** (`src/reminders/`) — background job that polls the NexoPRM iCalendar feed every 5 minutes and schedules Telegram pushes for upcoming occurrences via `at`. This is what makes "remind me tomorrow to text Cassie" (routed through `nexo-prm`) land on my phone without needing the `telegram-reminder` skill. Entry point `src/reminders/index.ts`; reconciler in `src/reminders/scheduler.ts`; minimal ICS parser + RRULE expander in `src/reminders/ics.ts`. Looks 25 hours forward and includes AI reminders, todo reminders, home items, and recurring tasks — skips meal plans, weekly briefs, and important dates (noise / already-delivered elsewhere). Timed events fire at DTSTART; all-day events fire at 8am America/Chicago. State lives at `data/reminders-state.json` (gitignored) mapping `uid::occurrenceTs → atJobId`, so reconciliation across ticks handles cancellations in PRM (item drops from feed → `atrm`'d within 5 min) and time edits (differ by `fireTs` → re-scheduled). Requires `NEXO_CALENDAR_FEED_URL` in `.env`. Dev: `npm run dev:reminders`.
