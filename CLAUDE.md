...
**Nexo** — a personal assistant with access to my custom NexoPRM platform. Telegram is the current UI: you message me and I handle everything from managing your contacts to organizing groceries, reminders, and plans.
...
- **`nexo-notebooks` skill** (`.claude/skills/nexo-notebooks/`) — manages notebooks (ongoing topics/threads): creating notebooks, adding notes and reflections, managing next actions, linking people. Notebooks are multi-week initiatives that accumulate notes and actions over time. Each note can be tagged for slicing long timelines (e.g., yearly recurring projects). Triggers: "add a note to my father-son trip notebook", "what notebooks do I have", "create a new notebook for X", "add a reflection about Y".
- **`nexo-plan` skill** (`.claude/skills/nexo-plan/`) — manages working notes (current plan) and todos: adding plan items, checking off tasks, managing headings, and the "debrief" todos rollup. Triggers: "what's on my plan", "add a todo", "debrief me", "check off X", "what are my open todos".
- **`nexo-meals` skill** (`.claude/skills/nexo-meals/`) — manages meals and food log: saving meal ideas, logging food intake, tracking calories. Triggers: "save this recipe", "log that I ate X", "what did I eat today", "add a meal idea".
- **`nexo-home` skill** (`.claude/skills/nexo-home/`) — manages home items (household chores and maintenance tasks). Triggers: "add a home task", "what home items do I have", "mark X as done", "add notes to home item".
- **`nexo-prm` skill** (`.claude/skills/nexo-prm/`) — manages areas of focus and stash (pocket knowledge base for non-person facts like products, places, gate codes). Triggers: "what are my focus areas", "remember that I like Reach floss", "save this cafe".
- **Briefing skills** — three-way family, all snapshot to `public/briefings/` (tracked in git; the skills auto-commit and push each snapshot so history follows the repo across machines):
  ...
  - For a lightweight todos-only rollup without snapshotting, the `nexo-plan` skill's "debrief" flow is the right call.
- **Briefing system** — pulls together a snapshot of your current state: open todos, plan items, reminders, recent moments, and important people. Used by the briefing skills to give you quick summaries.
- **`remember` skill** (`.claude/skills/remember/`) — turns casual "remember X" requests into durable notes committed into the repo. Routes facts to the right file under `docs/` (project docs, `docs/matt/*`, or `CLAUDE.md`), confirms wording and destination, then commits and pushes. For facts about a person already in PRM, it defers to `nexo-people` instead.
...
- **Reminders system** — background job that checks your reminders every 5 minutes and sends you Telegram notifications. This is what makes "remind me tomorrow to text Cassie" land on my phone. Looks ahead 25 hours and includes AI reminders, todo reminders, home items, and recurring tasks. Timed events fire at their scheduled time; all-day events fire at 8am CST. Recurring tasks can have lead times (e.g., a weekly task fires 3 days before the due date).
...
- **Send files back via Telegram** — When you ask for exports (lists, CSVs, reports, etc.), I can generate and send them as files. Default to files for anything long or structured. For quick reads that fit in a message, I'll just reply normally.
...
- **`nexo-notebooks`** — pass the full intent with context, e.g. `Add a note to the father-son trip notebook about visiting Lake Murray`, not just `add note`.
- **`nexo-plan`** — pass the full intent, e.g. `Add 'call the plumber' to Matt's plan` or `Debrief Matt on his open todos`.
- **`nexo-meals`** — pass the full intent, e.g. `Save 'Thai basil chicken' meal idea` or `Log that Matt ate a chicken salad for lunch`.
- **`nexo-home`** — pass the full intent, e.g. `Add 'replace smoke alarm batteries' to Matt's home items` or `Mark 'regrout shower' as done`.
- **`nexo-prm`** — pass the full intent with context about what to add/update (areas of focus or stash entries).
...
**"Reset" means run `npm run reset-session`** — this deletes the `.session-id` file (clearing Claude's conversation memory) and automatically restarts the agent. This is different from a regular restart which preserves session memory.
