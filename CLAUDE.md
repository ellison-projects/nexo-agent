...
**Nexo** — a personal assistant agent with access to my custom NexoPRM platform (via the `nexo-people` and `nexo-prm` skills). Telegram is the current UI: messages in, agent replies out. Under the hood each message becomes a `query()` call to the Claude Agent SDK, and the returned result is edited back into a placeholder message the bot posted.
