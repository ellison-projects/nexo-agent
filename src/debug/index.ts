...
async function poll(): Promise<void> {
      ...
      console.log(`Nexo-Dev listening for Telegram messages from chat ${debugEnv.telegramChatId}...`);
      await sendMessage(Number(debugEnv.telegramChatId), 'Nexo-Dev online 🛠️').catch(() => {});
      ...
      while (true) {
            try {
                  ...
                  for (const update of updates) {
                        ...
                        if (String(msg.chat.id) !== debugEnv.telegramChatId) {
                              ...
                              await sendMessage(
                                    Number(debugEnv.telegramChatId),
      ...
      while (true) {
            ...
            try {
                  ...
                  try {
                        ...
                        const reply = await askDebug(text, imagePath, imageUrl);
                        console.log(`Nexo-Dev: ${reply}`);
                        ...
                        const errText = 'Debug agent hit an error. Check pm2 logs nexo-debug-agent.';
                        if (placeholderId !== null) {
